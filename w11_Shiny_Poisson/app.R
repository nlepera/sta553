...
ui <- fluidPage(
  ...
  #well panel
    wellPanel(style = "background-color:#2f5c40; margin: 2px 1px 2px 1px",
  # Application title
    titlePanel(h1("Simulating the Standard Poisson Distribution", align = "center", style = "color: white; font-variant: small-caps; font-weight: bold"))),
...
server <- function(input, output) {
    ...
    output$poisPlot <- renderPlot({
      ...
      density.pois <- (dpois(pois.input, input$lambda))
      par(bg = "#90af9c")
      hist(pois.input, col = '#dde6bb', border = 'white',
      ...
      hist(density.pois, border="blue", col = "white", add = TRUE)})
